...
CREATE TABLE IF NOT EXISTS PATIENTS_REGISTRATION (
    PATIENT_NUMBER VARCHAR(30) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS VITALS_RECORDS (
    ...
    PRIMARY KEY (PATIENT_NUMBER, VISIT_DATE),
    CONSTRAINT FK_VITALS_PATIENT
        FOREIGN KEY (PATIENT_NUMBER)
        REFERENCES PATIENTS_REGISTRATION(PATIENT_NUMBER)
        ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS ASSESSMENT_RECORDS (
    ...
    PRIMARY KEY (PATIENT_NUMBER, VISIT_DATE),
    CONSTRAINT FK_GENERAL_ASSESS_PATIENT
        FOREIGN KEY (PATIENT_NUMBER)
        REFERENCES PATIENTS_REGISTRATION(PATIENT_NUMBER)
        ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS OVERWEIGHT_ASSESSMENT_RECORDS (
    ...
    COMMENTS TEXT NOT NULL,
    PRIMARY KEY (PATIENT_NUMBER, VISIT_DATE),
    CONSTRAINT FK_OVERWEIGHT_ASSESS_PATIENT
        FOREIGN KEY (PATIENT_NUMBER)
        REFERENCES PATIENTS_REGISTRATION(PATIENT_NUMBER)
        ON DELETE CASCADE
);
